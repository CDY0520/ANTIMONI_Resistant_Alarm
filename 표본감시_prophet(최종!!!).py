...
for var in external_vars:
    ext_df = df[['ds', var]].dropna().rename(columns={var: 'y'})
    model = Prophet()
    model.fit(ext_df)
...
# 6. 성능지표 계산
forecast_df = pd.merge(full_model_df[['ds', 'y']], forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']], on='ds', how='left')
...
plt.fill_between(plot_df['ds'], plot_df['yhat_lower'], plot_df['yhat_upper'], color='red', alpha=0.2, label='신뢰구간 (95%)')
plt.plot(plot_df['ds'], plot_df['y'], 'o-', label='실제값', color='blue')
plt.plot(plot_df['ds'], plot_df['yhat'], 'o--', label='예측값', color='red')
plt.title('표본감시 Prophet 예측 결과 (2021~2024.03)', fontsize=13)
plt.xlabel('날짜')
plt.ylabel('표본감시 건수')
...
forecast_df[save_cols].to_excel("표본감시_경보결과.xlsx", index=False)
...
plt.plot(observed['ds'], observed['y'], 'o-', color='blue', label='실제 표본감시 건수')
...
# 포맷
plt.title('표본감시 이상치 탐지 (2023-01 ~ 2024-01)', fontsize=14)
plt.xlabel('날짜')
plt.ylabel('표본감시 건수')
plt.xticks(rotation=45)
plt.legend()
...
# 엑셀 저장
component_df.to_excel("표본감시_구성요소_결과.xlsx", index=False)